# train_front_cv.py with ResNet34 and 384x384 resolution
import torch
...
for fold, (train_idx, val_idx) in enumerate(skf.split(np.zeros(len(targets)), targets)):
    ...
    plt.tight_layout()
    plt.savefig(f"results/confusion_matrices/front_fold{fold + 1}_confusion_matrix.png")
    plt.close()

    if val_acc > best_acc:
        best_acc = val_acc
        torch.save(model.state_dict(), f"model_front_best.pth")
    ...
    plt.tight_layout()
    plt.savefig(f"results/metrics/front_fold{fold + 1}_training_plot.png")
    plt.close()
